...
const Navbar = ({ getCartCount, isLoggedIn, handleLogout, userEmail }) => {
  ...
  useEffect(() => {
    ...
    document.body.style.overflow =
      isMenuOpen || isWishlistOpen || isUserProfileOpen ? "hidden" : "auto";
  ...
  const handleMouseEnter = (type) =>
    !isMobileView && setDropdowns({ ...dropdowns, [type]: true });
  const handleMouseLeave = (type) =>
    !isMobileView && setDropdowns({ ...dropdowns, [type]: false });

  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        isWishlistOpen &&
        wishlistModalRef.current &&
        !wishlistModalRef.current.contains(event.target)
      ) {
  ...
  useEffect(() => {
    ...
    setIsUserProfileOpen(
      storedLoginState && localStorage.getItem("isUserProfileOpen") === "true"
    );
  }, []);
          ...
          <ul className="nav-menu">
            ...
            >
              <>
                ...
                {(menu === "Accessories" || dropdowns.accessory) && (
                  <div className="underline"></div>
                )}
              </>
              {dropdowns.accessory && (
                <ul className="dropdown-content">
                  <li
                    onClick={() => handleDropdownItemClick("/accessories/bags")}
                  >
                    <Link to="/accessories/bags">Bags</Link>
                  </li>
                  <li
                    onClick={() =>
                      handleDropdownItemClick("/accessories/jewellery")
                    }
                  >
                  ...
                  <li
                    onClick={() =>
                      handleDropdownItemClick("/accessories/fragrance")
                    }
                  >
